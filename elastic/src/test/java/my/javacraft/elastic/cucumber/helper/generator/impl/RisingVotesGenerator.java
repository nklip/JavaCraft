...
public class RisingVotesGenerator implements VoteGenerator {
     ...
     *
     *   postId 31 → upvote% 72 → karma 44
     *   postId 32 → upvote% 73 → karma 46
     *   ...
     *   postId 40 → upvote% 81 → karma 62
     *
     * karma = 2 × upvotePercent − 100  (exact because isUpvote() produces a full
     * permutation of 0-99 over 100 users when gcd(31, 100) = 1).
     *
     * Starts at 72 % (not 71 %) to avoid a karma collision with HotVotesGenerator
     * whose ceiling is 71 % (karma 42).
     *
     ...
     *   - max karma (62) > max HotVotesGenerator karma (42) → posts 31-40 win Top MONTH.
     */
    @Override
    public void generatePostVotesInCsv() {
        ...
        for (int postId = 31; postId <= 40; postId++) {
            int upvotePercent = 72 + (postId - 31);    // 72% → 81%, unique per post
