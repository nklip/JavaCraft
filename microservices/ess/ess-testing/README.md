...
End-to-end integration tests for `ess-app` plus a set of standalone data downloaders
...
## 1. Quick Start
<sub>[Back to top](#ess-testing)</sub>
...
mvn -pl microservices/ess/ess-testing compile exec:java \
  -Dexec.mainClass=json.dev.nklip.javacraft.elastic.data.MoviesDownloader

# optional: write to a custom path
mvn -pl microservices/ess/ess-testing exec:java \
  -Dexec.mainClass=json.dev.nklip.javacraft.elastic.data.MoviesDownloader \
  -Dexec.args="/tmp/movies.json"
```

Default output path for each downloader:
`microservices/ess/ess-testing/src/test/resources/data/json/<dataset>.json`
...
```bash
mvn -pl microservices/ess/ess-testing test -Dtest="*DownloaderTest"
```
